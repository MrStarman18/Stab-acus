...
Date: May 21, 2022
Project: Stab-acus!
This program is used to run a game of Stab-acus*/
using namespace std;
    ...
    {   
        ...
        if (roundNumber == 1)    //input for first player
        {
            while (currentPlayer > 1)   //if invalid input, keep asking
            {
                ...
                cout << "I didn't get that. Let's try that again.\n"; //runs if invalid
            }
        }
        else if (roundNumber >= 2)   //player who just lost gets to start
            currentPlayer = !roundWinner;

        while (roundOver != true)
        {
            game.print("idle", currentPlayer);
            turnOver = false;
            ...
            while (turnOver != true)    //not allowed to skip turn using invalid inputs
            {
                ...
                if (thisTurn.getMoved() == false && thisTurn.getStabbed() == true && roundOver == false)
                {
                    game.print("idle", currentPlayer);
